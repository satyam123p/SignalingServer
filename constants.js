...
from controller.ResponseService import ResponseService

def lambda_handler(event, context):
    try:
        response_service = ResponseService.get_instance(event=event, context=context, is_ecs=False)
...
class ResponseService:
    ...
    @classmethod
    def get_instance(cls, event: Optional[Dict] = None, context: Any = None, is_ecs: Union[str, bool] = False) -> 'ResponseService':
        """Get or create the singleton instance of ResponseService."""
        if is_ecs:
            # For ECS, always create a new instance
            return cls(event, context, is_ecs)
        
        with cls._lock:
            if not cls._instance:
                cls._instance = cls(event, context, is_ecs)
            else:
                # Update the existing instance with new event data
                cls._instance._update_instance(event or {}, event.get('headers', {}) if event else {})
    ...
    def add_custom_response(self, code: Optional[Dict], params: Dict, response_body: Optional[Dict] = None) -> Dict:
        ...
        if code:
            ...
            message = ResponseCodes.UNKNOWN['MESSAGE']
            if params.get('type'):
                message += f" [{params['type']}]"
            if params.get('message'):
                message += f" ({params['message']})"
            response_body['resultMessage'] = message
            response_body['resultCode'] = ResponseCodes.UNKNOWN['CODE']
    ...
    def create_ok_http_response(self, response_body: Any) -> Union[None, Dict]:
        response_body = self.add_custom_response(ResponseCodes.OK, {}, response_body)
    ...
    def create_error_http_response(self, status_code: int, error_object: Exception, body: Optional[Dict] = None) -> Union[None, Dict]:
        ...
        code_obj = getattr(ResponseCodes, getattr(error_object, 'name', 'UNKNOWN'), ResponseCodes.UNKNOWN)
