...
async def get_system_tags(target: str, db_instance: DatabaseService, workspace_id: str) -> List[Dict]:
    try:
        system_tag_query = """
            ...
            WHERE TARGET = $1
        """
        system_tags = await db_instance.select_rows({
            "query": system_tag_query,
            "values": [target, workspace_id],
        })
        return system_tags
    except Exception as error:
        logger.error(f"[GET-SYSTEM-TAGS][ERROR] {error}, [STACK] {error.__traceback__}")
        raise

async def get_system_tags_by_keys(tag_keys: List[str], db_instance: DatabaseService, target: str) -> List[Dict]:
    query = f"SELECT * FROM UMS_SYSTEM_TAG WHERE TARGET = $1 AND KEY IN ({','.join(f'${i+2}' for i in range(len(tag_keys)))})"
    values = [target, *tag_keys]
    return await db_instance.select_rows({"query": query, "values": values})

async def is_exists_tag_data_in_workspace(db_instance: DatabaseService, type: str, tag_data_ids: List[str], workspace_id: str) -> bool:
    query = ""
    if type == 'TAG_ID':
        query = f"""
            ...
            WHERE RELATION.PLACE_ID = $1 AND TAG.TAG_ID IN ({','.join(f'${i+2}' for i in range(len(tag_data_ids)))})
            UNION
            SELECT TAGSET.TAGSET_ID, CATEGORY.TAGSET_CATEGORY_ID, TAG.TAG_ID, TAG.TAG_NAME, TAGSET.IS_GLOBAL
            FROM AMS_TAGSET TAGSET
            INNER JOIN AMS_TAGSET_CATEGORY CATEGORY ON TAGSET.TAGSET_ID = CATEGORY.TAGSET_ID
            RIGHT JOIN AMS_TAG TAG ON CATEGORY.TAGSET_CATEGORY_ID = TAG.TAGSET_CATEGORY_ID
            WHERE TAG.TAG_ID IN ({','.join(f'${i+2}' for i in range(len(tag_data_ids)))}) AND TAGSET.IS_GLOBAL = TRUE
        """
        ...
        query = f"""
            ...
            INNER JOIN AMS_TAGSET TAGSET ON TAGSET.TAGSET_ID = RELATION.TAGSET_ID
            INNER JOIN AMS_TAGSET_CATEGORY CATEGORY ON TAGSET.TAGSET_ID = CATEGORY.TAGSET_ID
            WHERE RELATION.PLACE_ID = $1 AND CATEGORY.TAGSET_CATEGORY_ID IN ({','.join(f'${i+2}' for i in range(len(tag_data_ids)))})
            UNION
            SELECT TAGSET.TAGSET_ID, CATEGORY.TAGSET_CATEGORY_ID, TAGSET.IS_GLOBAL
            FROM AMS_TAGSET TAGSET
            INNER JOIN AMS_TAGSET_CATEGORY CATEGORY ON TAGSET.TAGSET_ID = CATEGORY.TAGSET_ID
            WHERE CATEGORY.TAGSET_CATEGORY_ID IN ({','.join(f'${i+2}' for i in range(len(tag_data_ids)))}) AND TAGSET.IS_GLOBAL = TRUE
        """
        ...
        query = f"""
            ...
            FROM (
                ...
                WHERE SYSTEM_TAG.SYSTEM_TAG_ID IN ({','.join(f'${i+2}' for i in range(len(tag_data_ids)))}) AND TYPE != 'CUSTOM'
            ...
            FULL JOIN (
                ...
                WHERE PLACE_ID = $1 AND SYSTEM_TAG.SYSTEM_TAG_ID IN ({','.join(f'${i+2}' for i in range(len(tag_data_ids)))}) AND TYPE = 'CUSTOM'
            ) AS CUSTOM_SYSTEM_TAG
            ON NOT_CUSTOM_SYSTEM_TAG.SYSTEM_TAG_ID = CUSTOM_SYSTEM_TAG.SYSTEM_TAG_ID
        """

    result = await db_instance.select_rows({
        "query": query,
        "values": [workspace_id, *tag_data_ids],
    })
...
class TagsService:
    ...
    delete_tag_by_category = delete_tag_by_category
    copy_tags = copy_tags
    get_system_tags = get_system_tags
    get_system_tags_by_keys = get_system_tags_by_keys
    is_exists_tag_data_in_workspace = is_exists_tag_data_in_workspace
